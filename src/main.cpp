...
// Meter calibration
#define GALLONS_PER_PULSE  0.001f
...
// PubSubClient default buffer (128 bytes) is too small for HA discovery payloads.
...
void publishState() {
  ...
  if (elapsed >= WINDOW_MS) {
    ...
    flowGpmSmoothed = 0.5f * flowGpmSmoothed + 0.5f * gpm;
...
#endif
  // Ensure MQTT buffer can hold discovery JSON payloads before first publish.
